...
| 轮次 | 本轮后累计信息覆盖率目标 | Demo 默认值 | 说明 |
|------|--------------------------|-------------|------|
| 第 1 轮后 | 10%～25% | 18% | 只建立基础判断，不让玩家过早算清楚 |
| 第 2 轮后 | 30%～50% | 40% | 能开始根据图鉴排除候选，形成明显报价调整 |
| 第 3 轮后 | 约 70% | 70% | 接近最终决策所需信息，但仍保留约 30% 不确定性 |
...
因此本 Demo 的信息点采用**区间内随机目标**，而不是固定脚本：

| 轮次 | 公开信息 | 私有信息 | 本轮信息点 | 累计 |
|------|----------|----------|------------|------|
| 第 1 轮 | 约 40%～60% | 约 40%～60% | 随机凑到 10～25 | 10%～25% |
| 第 2 轮 | 约 50% | 约 50% | 随机凑到累计 30～50 | 30%～50% |
| 第 3 轮 | 约 40% | 约 60% | 随机凑到累计 65～75 | 约 70% |

实际信息文本仍按场次与随机集装箱生成；Demo 会优先用多条弱信息拼成一条“公开信息/私有信息”，而不是强行把弱信息抬高点数。

信息类型不做固定轮次绑定：

- 第 1 轮可以给总件数，也可以给轮廓、品质，甚至直接给一个具体物品。
- 第 2 / 第 3 轮也不强制必须按 `轮廓 → 品质 → 具体物品` 递进。
- 唯一硬约束是：每轮仍然只发 1 条公开信息和 1 条私有信息，并且累计覆盖率满足本节目标区间。
